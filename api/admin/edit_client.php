...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(!empty($data->client_id) && !empty($data->username)) {
        ...
        if(!empty($data->password)) {
            ...
            $stmt = $db->prepare("UPDATE users SET username = :username, password = :password, full_name = :full_name, country = :country, state = :state, city = :city, company_name = :company_name WHERE id = :id AND role = 'client'");
            $stmt->bindParam(':password', $pwd);
        } else {
            $stmt = $db->prepare("UPDATE users SET username = :username, full_name = :full_name, country = :country, state = :state, city = :city, company_name = :company_name WHERE id = :id AND role = 'client'");
        }
